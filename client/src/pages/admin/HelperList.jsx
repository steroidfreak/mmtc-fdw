// client/src/admin/HelperList.jsx
...
export default function HelperList() {
    ...
    // modal/form state
    const [openId, setOpenId] = useState(null); // null=create, string=edit id, undefined=closed
    const [refreshKey, setRefreshKey] = useState(0); // force HelperForm to re-mount on openId change
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <button className="bg-blue-600 text-white px-3 py-2 rounded" onClick={openCreate}>+ New Helper</button>
                </div>
            </div>
                            ...
                            <tr key={h._id} className="border-b hover:bg-gray-50">
                                <td className="px-2 py-2">
                                    {Array.isArray(h.photos) && h.photos[0] ? (
                                        <img src={h.photos[0]} alt="" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 4 }} />
                                    ) : (
                                        <img src="/placeholder-helper.png" alt="" style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 4, opacity: 0.85 }} />
                                    )}
                                </td>
                                ...
                                <td className="px-2 py-2">
                                    <button className="px-2 py-1 border rounded" onClick={() => openEdit(h._id)}>Edit</button>
                                    <button className="px-2 py-1 border rounded ml-2" onClick={() => removeHelper(h._id)}>Delete</button>
                                </td>
                            </tr>
                        ))}
                        {!loading && (!items || items.length === 0) && (
                            <tr>
                                <td className="px-2 py-4 text-gray-600" colSpan={9}>No helpers found.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                ...
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded shadow-xl p-4 max-w-2xl w-full">
                        <div className="flex items-center justify-between mb-2">
                            <h3 className="text-lg font-semibold">{openId ? 'Edit Helper' : 'Create Helper'}</h3>
                            <button className="text-gray-600" onClick={closeForm}>✕</button>
                        </div>
                        <HelperForm key={refreshKey} helperId={openId || undefined} onSaved={onSaved} />
                    </div>
                </div>
            )}
        </div>
